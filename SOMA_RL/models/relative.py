...
class fRelative(RLToolbox, nn.Module):

    """
    Reinforcement Learning Model: Forward-Thinking Relative Model
    Different from the Relative model in that it updates the state value before the prediction error is computed
    ...
    def compute_context_prediction_error(self, state):
        if self.training == 'torch':
            state['context_prediction_errors'] = state['context_reward'] - state['context_value'].detach()
        else:
            state['context_prediction_errors'] = state['context_reward'] - state['context_value']
    ...
    def forward(self, state, phase = 'learning'):
        if phase == 'learning':
            ...
            state = self.get_context_value(state)
            state = self.select_action(state)
            state = self.compute_context_prediction_error(state)
            state = self.update_context_values(state)
            state = self.compute_prediction_error(state)
            self.update_model(state)
        else:
            state = self.get_final_q_values(state)
